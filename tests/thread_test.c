...
static void *_mode_test_thread(void *arg)
{
    ...
    if (sys_thread_mode() != SYS_THREAD_MODE_NONE) {
        *reason = "Subthread expected default mode, failed!";
        return 0;
    }

    sys_thread_mode_t old_mode = sys_thread_set_mode(SYS_THREAD_MODE_LISTENER);
    if (old_mode != SYS_THREAD_MODE_NONE) {
        *reason = "Subthread expected old mode to be default, failed!";
        return 0;
    }

    if (sys_thread_mode() != SYS_THREAD_MODE_LISTENER) {
