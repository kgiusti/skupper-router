...
//
static sys_thread_t           _main_thread_id = {.role = SYS_THREAD_MAIN, .name = "main"};
...
sys_thread_t *sys_thread(sys_thread_role_t role, void *(*run_function)(void *), void *arg)
{
    ...
    (void) rc;
    assert(rc == 0);
    rc = pthread_setname_np(thread->thread, thread->name);
    assert(rc == 0);
...
sys_thread_mode_t sys_thread_set_mode(sys_thread_mode_t new_mode)
{
    sys_thread_mode_t old_mode = _self->mode;
    _self->mode = new_mode;
    return old_mode;
}
