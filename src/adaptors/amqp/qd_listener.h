...
// associate a new connection with the parent listener
void qd_listener_add_connection(qd_listener_t *li, qd_connection_t *ctx);

// disassociate the connection with its parent listener
// NOTE WELL: may free the listener if this connection is holding the last reference
// to it
void qd_listener_release_connection(qd_listener_t *li, qd_connection_t *qd_conn);
