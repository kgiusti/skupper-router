...
static uint64_t qdr_http_deliver(void *context, qdr_link_t *link, qdr_delivery_t *delivery, bool settled)
{
    ...
    qd_log(http2_adaptor->log_source, QD_LOG_DEBUG, DLV_FMT " qdr_http_deliver", DLV_ARGS(delivery));
...
static bool push_rx_buffer_to_nghttp2(qdr_http2_connection_t *conn, uint8_t *buf, size_t size)
{
    ...
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_DEBUG, "[C%" PRIu64 "] nghttp2_session_mem_recv rv=%i",
           conn->conn_id, rv);
...
static int handle_incoming_http(qdr_http2_connection_t *conn)
{
    ...
    if (conn->require_tls) {
        ...
        //     1) 2c. SETTINGS frame ack=false(this is the router's inital settings frame) 2d. GET request
    ...
    if (conn->ingress) {
        ...
        if (conn->require_tls) {
            ...
            if (conn->tls) {
                ...
                qd_log(http2_adaptor->log_source, QD_LOG_ERROR,
...
qd_http_listener_t *qd_http2_configure_listener(qd_dispatch_t *qd, qd_http_adaptor_config_t *config, qd_entity_t *entity)
{
    ...
    if (li->config->adaptor_config->ssl_profile_name) {
        li->tls_domain = qd_tls_domain(li->config->adaptor_config, qd, http2_adaptor->log_source, protocols,
    ...
    qd_log(http2_adaptor->log_source, QD_LOG_INFO, "Configured http2_adaptor listener on %s",
