...
#include "qpid/dispatch/tls_raw_io.h"
#include "private.h"
                        ...
                        uint64_t                        *input_data_count)
{
    ...
    const bool debug = qd_log_enabled(session->log_module, QD_LOG_DEBUG);
    ...
    do {
        ...
        capacity = pn_tls_get_encrypt_output_buffer_capacity(session->pn_tls_session);
        if (debug && capacity) {
            qd_log_impl(session->log_module, QD_LOG_DEBUG, __FILE__, __LINE__, "[C%" PRIu64 "] giving %zu encrypt output buffers", session->conn_id, capacity);
        }
        while (capacity-- > 0) {
            _pn_buf_desc_give_buffer(&pn_buf_desc, qd_buffer());
            given = pn_tls_give_encrypt_output_buffers(session->pn_tls_session, &pn_buf_desc, 1);
            (void) given;
            assert(given == 1);
        }
        capacity = pn_tls_get_decrypt_output_buffer_capacity(session->pn_tls_session);
        if (debug && capacity) {
            qd_log_impl(session->log_module, QD_LOG_DEBUG, __FILE__, __LINE__, "[C%" PRIu64 "] giving %zu decrypt output buffers", session->conn_id, capacity);
        }
        while (capacity-- > 0) {
            _pn_buf_desc_give_buffer(&pn_buf_desc, qd_buffer());
            given = pn_tls_give_decrypt_output_buffers(session->pn_tls_session, &pn_buf_desc, 1);
            (void) given;
            assert(given == 1);
        }
        ...
        if (pn_tls_is_secure(session->pn_tls_session)) {
            if (take_output_cb) {
                capacity = pn_tls_get_encrypt_input_buffer_capacity(session->pn_tls_session);
                if (capacity > 0) {
                    qd_buffer_list_t ubufs = DEQ_EMPTY;
                    if (debug) {
                        qd_log_impl(session->log_module, QD_LOG_DEBUG, __FILE__, __LINE__, "[C%" PRIu64 "] encrypt input capacity = %zu bufs", session->conn_id, capacity);
                    }
                    int64_t out_octets = take_output_cb(take_output_context, &ubufs, capacity);
                    if (out_octets > 0) {
                        assert(!DEQ_IS_EMPTY(ubufs) && DEQ_SIZE(ubufs) <= capacity);
                        qd_log(session->log_module,
                               QD_LOG_DEBUG,
                        ...
                        while (abuf) {
                            ...
                            given = pn_tls_give_encrypt_input_buffers(session->pn_tls_session, &pn_buf_desc, 1);
                            assert(given == 1);
                            abuf = DEQ_HEAD(ubufs);
                        }
                        ...
                        if (out_octets == QD_IO_EOS) {
                            ...
                            pn_tls_close_output(session->pn_tls_session);
                        }
                        session->output_eos = true;
                        qd_log(session->log_module, QD_LOG_DEBUG, "[C%" PRIu64 "] outgoing stream EOS signalled: closing TLS output",
                               session->conn_id);
                    }
                }
            } else if (debug) {
                // take_output_cb is not set
                qd_log_impl(session->log_module, QD_LOG_DEBUG, __FILE__, __LINE__, "[C%" PRIu64 "] output data blocked", session->conn_id);
            }
        }
        ...
        capacity = pn_tls_get_decrypt_input_buffer_capacity(session->pn_tls_session);
        if (capacity > 0) {
            if (debug) {
                qd_log_impl(session->log_module, QD_LOG_DEBUG, __FILE__, __LINE__, "[C%" PRIu64 "] decrypt input capacity = %zu bufs", session->conn_id, capacity);
            }
            if (!pn_tls_is_secure(session->pn_tls_session) || input_data != 0) {
                size_t pushed = 0;
                total_octets  = 0;
                while (pushed < capacity) {
                    ...
                    if (took != 1) {
                        ...
                        given = pn_tls_give_decrypt_input_buffers(session->pn_tls_session, &pn_buf_desc, 1);
                        assert(given == 1);
                        ++pushed;
                    } else {
        ...
        if (!session->tls_error) {
            const bool check_if_secure = session->on_secure_cb && !pn_tls_is_secure(session->pn_tls_session);
            int        err             = pn_tls_process(session->pn_tls_session);
            if (err) {
                session->tls_error = true;
                qd_log(session->log_module, QD_LOG_DEBUG, "[C%" PRIu64 "] pn_tls_process failed: error=%d", session->conn_id,
                       err);
            } else if (check_if_secure && pn_tls_is_secure(session->pn_tls_session)) {
        ...
        if (capacity > 0) {
            ...
            if (debug) {
                qd_log_impl(session->log_module, QD_LOG_DEBUG, __FILE__, __LINE__, "[C%" PRIu64 "] raw write capacity=%zu bufs", session->conn_id, capacity);
            }
            while (pushed < capacity && pn_tls_take_encrypt_output_buffers(session->pn_tls_session, &pn_buf_desc, 1) == 1) {
                total_octets += pn_buf_desc.size;
            ...
            taken = 0;
            while (pn_tls_take_encrypt_output_buffers(session->pn_tls_session, &pn_buf_desc, 1) == 1) {
        ...
        if (input_data) {
            total_octets = 0;
            taken        = 0;
            while (pn_tls_take_decrypt_output_buffers(session->pn_tls_session, &pn_buf_desc, 1) == 1) {
            ...
            if (taken) {
                ...
                *input_data_count += total_octets;
                qd_log(session->log_module, QD_LOG_DEBUG,
                       ...
                       session->conn_id, total_octets, taken);
            }
        } else if (debug) {
            qd_log_impl(session->log_module, QD_LOG_DEBUG, __FILE__, __LINE__, "[C%" PRIu64 "] input_data blocked", session->conn_id);
        }
        ...
        taken = 0;
        while (pn_tls_take_encrypt_input_buffers(session->pn_tls_session, &pn_buf_desc, 1) == 1) {
            qd_buffer_t *abuf = (qd_buffer_t *) pn_buf_desc.context;
            assert(abuf);
            qd_buffer_free(abuf);
            ++taken;
        }
        while (pn_tls_take_decrypt_input_buffers(session->pn_tls_session, &pn_buf_desc, 1) == 1) {
            qd_buffer_t *abuf = (qd_buffer_t *) pn_buf_desc.context;
            assert(abuf);
            qd_buffer_free(abuf);
            ++taken;
    ...
    //
    if (session->tls_error && !pn_tls_is_decrypt_output_pending(session->pn_tls_session)) {
        char buf[1024] = "Unknown protocol error";  // default error msg
        int  err = pn_tls_get_session_error(session->pn_tls_session);
        if (err) {
            int rc = snprintf(buf, sizeof(buf), "Code: %d ", err);
            if (rc > 0 && rc < sizeof(buf)) {
                pn_tls_get_session_error_string(session->pn_tls_session, &buf[rc], sizeof(buf) - rc);
            }
        }
        ...
        qd_log(session->log_module, QD_LOG_WARNING, "[C%" PRIu64 "] TLS connection failed: %s", session->conn_id, buf);
        return -1;
    }

    // check if the output (encrypt) side is done
    //
    if (session->output_eos && !pn_tls_is_encrypt_output_pending(session->pn_tls_session)) {
        ...
        if (!pn_raw_connection_is_write_closed(raw_conn)) {
            qd_log(session->log_module, QD_LOG_DEBUG,
                   "[C%" PRIu64 "] TLS output closed - closing write side of raw connection", session->conn_id);
    ...
    //
    if (pn_tls_is_input_closed(session->pn_tls_session)) {
        ...
        if (!pn_raw_connection_is_read_closed(raw_conn)) {
            qd_log(session->log_module, QD_LOG_DEBUG,
                   "[C%" PRIu64 "] TLS input closed - closing read side of raw connection", session->conn_id);
            pn_raw_connection_read_close(raw_conn);
        }
        while (pn_raw_connection_take_read_buffers(raw_conn, &pn_buf_desc, 1) == 1) {
            qd_buffer_free(_pn_buf_desc_take_buffer(&pn_buf_desc));
        }
    } else if (session->raw_read_drained && !pn_tls_is_decrypt_output_pending(session->pn_tls_session)) {
        // Unclean close: remote simply dropped the connection. Done reading remaining decrypted output.
        //
        session->input_drained = true;
        qd_log(session->log_module, QD_LOG_DEBUG,
               "[C%" PRIu64 "] TLS input closed", session->conn_id);
    }
...
{
    if (!session || !session->pn_tls_session)
        return false;
...
bool qd_tls2_session_is_secure(const qd_tls2_session_t *session)
{
    if (!session || !session->pn_tls_session)
        return false;
    return pn_tls_is_secure(session->pn_tls_session);
}
...
{
    assert(session);
    *close_notify = pn_tls_is_input_closed(session->pn_tls_session);
...
uint64_t qd_tls2_session_encrypted_output_octet_count(const qd_tls2_session_t *session)
{
    assert(session);
...
uint64_t qd_tls2_session_encrypted_input_octet_count(const qd_tls2_session_t *session)
{
    assert(session);
