...
static void qdr_add_router_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    do {
        if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "add_router: Router maskbit out of range: %d",
                   router_maskbit);
            break;
        }

        if (core->routers_by_mask_bit[router_maskbit] != 0) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "add_router: Router maskbit already in use: %d",
                   router_maskbit);
            break;
        }
...
static void qdr_del_router_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "del_router: Router maskbit out of range: %d",
               router_maskbit);
        return;
    }

    if (core->routers_by_mask_bit[router_maskbit] == 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "del_router: Deleting nonexistent router: %d",
               router_maskbit);
        return;
    }
...
static void qdr_set_link_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_link: Router maskbit out of range: %d", router_maskbit);
        return;
    }

    if (conn_maskbit >= qd_bitmask_width() || conn_maskbit < 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_link: Link maskbit out of range: %d", conn_maskbit);
        return;
    }
    ...
    if (core->routers_by_mask_bit[router_maskbit] == 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_link: Router not found");
        return;
    }
...
static void qdr_remove_link_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "remove_link: Router maskbit out of range: %d",
               router_maskbit);
        return;
    }
...
static void qdr_set_next_hop_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_next_hop: Router maskbit out of range: %d",
               router_maskbit);
        return;
    }

    if (nh_router_maskbit >= qd_bitmask_width() || nh_router_maskbit < 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_next_hop: Next hop router maskbit out of range: %d",
               router_maskbit);
        return;
    }

    if (core->routers_by_mask_bit[router_maskbit] == 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_next_hop: Router not found");
        return;
    }

    if (core->routers_by_mask_bit[nh_router_maskbit] == 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_next_hop: Next hop router not found");
        return;
    }
...
static void qdr_remove_next_hop_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "remove_next_hop: Router maskbit out of range: %d",
               router_maskbit);
        return;
    }
...
static void qdr_set_cost_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
        qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_cost: Router maskbit out of range: %d", router_maskbit);
        return;
    }
...
static void qdr_set_valid_origins_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    do {
        if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_valid_origins: Router maskbit out of range: %d",
                   router_maskbit);
            break;
        }

        if (core->routers_by_mask_bit[router_maskbit] == 0) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "set_valid_origins: Router not found");
            break;
        }
...
static void qdr_flush_destinations_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    do {
        if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "flush_destinations: Router maskbit out of range: %d",
                   router_maskbit);
            break;
        }
        ...
        qdr_node_t *rnode = core->routers_by_mask_bit[router_maskbit];
        if (rnode == 0) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "flush_destinations: Router not found");
            break;
        }
...
static void qdr_mobile_seq_advanced_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    do {
        if (router_maskbit >= qd_bitmask_width() || router_maskbit < 0) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "seq_advanced: Router maskbit out of range: %d",
                   router_maskbit);
            break;
        }
        ...
        qdr_node_t *rnode = core->routers_by_mask_bit[router_maskbit];
        if (rnode == 0) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_CRITICAL, "seq_advanced: Router not found");
            break;
        }
...
static void qdr_subscribe_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    if (!discard) {
        ...
        qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "In-process subscription %c/%s", aclass, astring);
        free(astring);
