 ...
 *
 * Note that the update callback is invoked on the management thread while the qd_tls_config is locked. This prevents
 * new TLS sessions from being created using the updated configuration until after the update callback returns.
 *
