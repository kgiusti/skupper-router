...
def main() -> int:
    ...
    pathy_mcpathface = pathlib.Path(build_dir, 'Testing', 'Temporary', 'LastTestsFailed.log')
    if not pathy_mcpathface.exists():
        logging.debug(f"File {pathy_mcpathface} does not exist - nothing to do")
        return 0

    with pathy_mcpathface.open('rt') as f:
        failed_tests = gha_tools.parse_last_tests_failed_log(f)
    failed_test_names = {test[1] for test in failed_tests}
    logging.debug(f"Failed tests: %s", failed_test_names)
